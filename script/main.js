    ...
    const DBService = class {
        ...
        getTestCard = async () => await this.getData('card.json');
        ...
        getTop = item => {
            this.temp = `${server}/tv/${item}?api_key=${API_KEY}&language=ru-RU`;
            return this.getData(this.temp);
        };
    };
    ...
    const renderCard = response => {
        ...
        if (response.total_pages > 1 && response.total_pages < 20 ) {
...
document.addEventListener('DOMContentLoaded', () => { 
    fetch(`${server}/tv/airing_today?api_key=${API_KEY}&language=ru`)
    .then(res => {
        tvShows.append(loading);
        if (res.ok) {
            return res.json();
